    ...
    public ResponseResult qqLogin(HttpServletRequest request, HttpServletResponse response){
        ...
        try {
            ...
            if(null != user || StringUtils.isNotBlank(user.getUserName()) ){
                //查询权限信息
                userAuthorityList = (List)redisTemplate.opsForValue().get(user.getUserName());
                if(null == userAuthorityList || userAuthorityList.size() <=0){

                    SysUser sysUser2 = sysApiService.getAllApi(user);
            ...
            if(userAuthorityList == null){
                ...
                userAuthorityList.add("guest");//游客
            }
            ...
            dataMap.put("currentAuthority", null);//权限
            msgMap.put(ResponseData.RESPONE_CODE_ERROR,"使用qq失败，请改用其它方式登录");//返回代码
            rr.setData(dataMap);//返回数据
            rr.setMsgMap(msgMap);//返回信息
            rr.setSuccess(false);
            e.printStackTrace();
